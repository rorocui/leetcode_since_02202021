...
void myLinkedListAddAtTail(MyLinkedList* obj, struct StringTreeNode* root) 
{
    ...
    if(obj->val)
    {
        ...
        end->val = malloc(STRING_SIZE * sizeof(char));
    ...
    else
    {
        ...
        obj->val = malloc(STRING_SIZE * sizeof(char));
...
void treeHashSetAdd(TreeHashSet* obj, struct StringTreeNode* root) 
{
    ...
    myLinkedListAddAtTail(obj->arr[strlen(root->stree)], root);
}
...
struct TreeNode* treeHashSetContain(TreeHashSet* obj, struct StringTreeNode* root, int* checked)
{
    ...
    return myLinkedListContain(obj->arr[strlen(root->stree)], root, checked); 
}
...
/* 175-1344(n/a)-660.4 , on Jun 28th, 2021 */
