...
{
    MinStack *stack_room = NULL;
    bool* room_visited = NULL;
    int i;
    int room_idx = 0;

    stack_room = minStackCreate();
    room_visited = calloc(roomsSize, sizeof(bool));
    room_visited[0] = true;

    for(i = 0; i < roomsColSize[0]; i++)
        minStackPush(stack_room, rooms[0][i]);

    while(!minStackEmpty(stack_room))
    {
        room_idx = minStackPop(stack_room);
        //printf("open room %d\n", room_idx);

        if(!room_visited[room_idx]) 
        {
		    room_visited[room_idx] = true;
		    for(i = 0; i < roomsColSize[room_idx]; i++)
		    {
                if(!room_visited[rooms[room_idx][i]]) 
		            minStackPush(stack_room, rooms[room_idx][i]);
		    }
        }
    }
    for(i = 0; i < roomsSize; i++)
        if(!room_visited[i])
        {
            //printf("room %d is not open\n", i);
            minStackFree(stack_room);
            free(room_visited);
            return false;
        }
...
/* 67 test cases in 20ms(<7%), in 6.9MB on May 27th, 2021 */
